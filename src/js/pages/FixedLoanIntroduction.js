...
let FixedLoanIntroduction = React.createClass({
  getInitialState() {
    return {
      data: FixedLoanIntroductionStore.getAll(),
      isMaskOpen: false,
    ...
    if (e.title === "分享") {
      if (isLogin) {
        ...
        window.history.replaceState(
          null,
          "",
          "#/fixedLoanIntroduction?&productId=" +
            productId +
            "&type=" +
            type +
            "&inviteCode=" +
            ...
            <Tabs defaultActiveKey={0}>
              <Tabs.Item title="项目信息" key={0} navStyle={null}>
                <Group noPadded={false}>
                  ...
                  <div className="content">
                    “好采投”为饭米粒理财根据投资方20年经营大数据严格筛选的优质借款人借款需求，每一个项目都经过现场勘查，并提供第三方担保人实力担保，对于贷后资金管理亦建立了一套严谨的监控体系，确保资金安全。
                  </div>
                </Group>
                <Group noPadded={false}>
                  <h6>
  ...
  componentDidMount() {
    ...
    FixedLoanIntroductionStore.bind(
      "change",
      function() {
...
FixedLoanIntroduction.contextTypes = {

...
let  NYDLoanIntroductionStore={
    _all:{
        ...
        repayType:"xxhb",//还款方式，oneOf["xxhb","debx","fddebx"],分别代表“先息后本”,“等额本息”和“分段式的等额本息”
        repaymentPeriod:""//分段的值，oneOf['twin','season'],分别是双月还，季季还
    },
    updateAll(source){
    ...
    extractGuaranteeLicenseListAndUpdate(data){
        ...
        for(let i=0;i<data.introductionList.length;i++){
            ...
            if(["担保情况","担保企业","担保措施","担保情况："].indexOf(labelText) > -1){
...
MicroEvent.mixin(NYDLoanIntroductionStore);


appDispatcher.register(function(payload){
    switch(payload.actionName){
        case "nydLoanIntroduction_getDataFromServer":
            ajax({
                ...
                success:function(rs){
                    ...
                    if(rs.code === 0){
                        ...
                        NYDLoanIntroductionStore.updateAll(NYDLoanIntroductionStore.processData(source));
                        NYDLoanIntroductionStore.trigger("change");
                        ...
                        ajax({
                            ...
                            success:function(rs){
                                if(rs.code === 0){
                                    NYDLoanIntroductionStore.extractLicenseListAndUpdate(rs.data.list);
                                    NYDLoanIntroductionStore.trigger("change");
                                }
                            }
                        });
                        ...
                        ajax({
                            ...
                            success:function(rs){
                                if(rs.code === 0){
                                    NYDLoanIntroductionStore.extractGuaranteeLicenseListAndUpdate(rs.data);
                                    NYDLoanIntroductionStore.trigger("change");
                                }
                            }
                        });
                        ...
                        NYDLoanIntroductionStore.trigger("getDataFailed");
                    }
                }
            });
            //获取标的借款人信息
                ...
                success:function(rs){
                    if(rs.code === 0){
                        if(rs.data.hasOwnProperty("enterprise")){
                            NYDLoanIntroductionStore.updateAll({
            ...
            ajax({
                ...
                success:function(rs){
                    if(rs.code === 0){
                        ...
                        let riskLevelScore=[data.zcScore,data.dbScore,data.zjScore,data.hkScore,data.xyScore,data.rzScore];//风险分数，依次是"资产状况", "担保情况", "资金保障", "还款保障", "信用历史", "认证信息"
                        NYDLoanIntroductionStore.updateAll({
...
module.exports=NYDLoanIntroductionStore;
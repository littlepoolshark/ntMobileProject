...
var FixedLoanInvestmentRecordStore={
    _all:{
        applyingList:[],//加入中列表
        preRepayList:[],//回款中列表
        clearingList:[],//已结算列表
    ...
    processData(listType,list){
        ...
        for(let i=0;i<list.length;i++){
            newList[i]={
                ...
                time:list[i].repayTime,
                investAmount:list[i].principal,//投资金额
                ...
                interestRate:!!list[i].interestRate ? list[i].interestRate + "%" : 0,//加息券的年化lilv
...
MicroEvent.mixin(FixedLoanInvestmentRecordStore);

appDispatcher.register(function(payload){
    let currListType;
    switch(payload.actionName){
        case "getInitialData_fixedLoanInvestmentRecord":
            //获取加入中列表
            ajax({
                ciUrl:"/invest/v2/projectInvestment",
                data:{
                    status:"orderring",
                    reqPageNum:1
                },
                success(rs){
                    if(rs.code === 0){
                        if(rs.data.list.length >0){
                            FixedLoanInvestmentRecordStore.updateAll({
                                listType:"applying",
                                list:rs.data.list,
                                pageIndex:rs.data.pageIndex
                            });
                            FixedLoanInvestmentRecordStore.trigger("change");
                        }else {
                            FixedLoanInvestmentRecordStore.trigger("noDataTemporarily","applying");
                        }
                ...
                success(rs){
                    if(rs.code === 0){
                        if(rs.data.list.length > 0){
                            ...
                            FixedLoanInvestmentRecordStore.trigger("noDataTemporarily","repaying");
                        }
                ...
                success(rs){
                    if(rs.code === 0){
                        if(rs.data.list.length > 0){
                            ...
                            FixedLoanInvestmentRecordStore.trigger("noDataTemporarily","clearing");
                        }
            ...
            break;
        case "getNextPage_fixedLoanInvestmentRecord":
            currListType=payload.data.currListType;
            ajax({
                ciUrl:"/invest/v2/projectInvestment",
                data:{
                    status:FixedLoanInvestmentRecordStore.getListStatusByListType(currListType),
                    reqPageNum:FixedLoanInvestmentRecordStore.getCurrPageIndex(currListType)+1
                },
                success(rs){
                    if(rs.code === 0){
                        if(rs.data.list.length >0){
                            ...
                            FixedLoanInvestmentRecordStore.trigger("noMoreData");
                        }
                    }
                }
            });
...
module.exports=FixedLoanInvestmentRecordStore;